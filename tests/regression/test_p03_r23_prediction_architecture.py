...
def test_p03_r23_engine_version():
    assert PREDICTION_ENGINE_VERSION == "P0.3-R25"

def test_p03_r23_registry_completeness():
...
def test_p03_r23_request_normalization_and_determinism():
    ...
    fp1 = prediction_request_fingerprint(req1)
    fp2 = prediction_request_fingerprint(req2)
    assert fp1 == fp2

def test_p03_r23_missing_selected_date_fails():
    dt = datetime(1990, 9, 10, 14, 30)
    chart = calculate_canonical_chart(dt, 10.5276, 76.2144, "Asia/Kolkata")
...
def test_p03_r23_unsupported_event_fails():
    dt = datetime(1990, 9, 10, 14, 30)
    chart = calculate_canonical_chart(dt, 10.5276, 76.2144, "Asia/Kolkata")
...
def test_p03_r23_domain_mismatch_fails():
    dt = datetime(1990, 9, 10, 14, 30)
    chart = calculate_canonical_chart(dt, 10.5276, 76.2144, "Asia/Kolkata")
...
def test_p03_r23_natal_date_invariance():
...
def test_p03_r23_cache_fail_closed():
    dt = datetime(1990, 9, 10, 14, 30)
    chart = calculate_canonical_chart(dt, 10.5276, 76.2144, "Asia/Kolkata")
...
def test_p03_r23_event_differentiation():
    dt = datetime(1990, 9, 10, 14, 30)
    chart = calculate_canonical_chart(dt, 10.5276, 76.2144, "Asia/Kolkata")
...
def test_p03_r23_score_reconstruction():
    dt = datetime(1990, 9, 10, 14, 30)
    chart = calculate_canonical_chart(dt, 10.5276, 76.2144, "Asia/Kolkata")
