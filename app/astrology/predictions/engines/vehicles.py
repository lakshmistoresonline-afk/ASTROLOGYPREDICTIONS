...
class VehiclesPredictionEngine:
    """
    V3 Authoritative Vehicles & Mobility Engine.
    Consumes canonical evaluate_natal_promise() as the single source of structural vehicle/mobility promise.
    """
    ...
    def get_prediction(chart: CanonicalChart, selected_date: datetime, event_type: str = None) -> DomainPrediction:
        ...
        if event_type:
            ...
            np_res = evaluate_natal_promise(chart, "VEHICLES", event_type)
        ...
        dasha_evidence = CorroborationEngine.audit_dasha_activation(dasha, chart, relevant_lords, "mobility")
        if dasha_evidence:
            evidence.extend(dasha_evidence)
        else:
            evidence.append(CorroborationEngine.create_evidence(
                "DASHA_FOUNDATION",
                "STABILITY PHASE: Life-period focuses on maintenance of existing conveyances.",
                60.0, group="SECONDARY"
            ))

        # 3. TIMING
        window = timing_engine.calculate_window(chart, ["Venus", "Mars", l4_name], [4, 2],
                                                calculation_date=selected_date, domain="Vehicles & Mobility")
        ...
        summary_template = (
            "Mobility and vehicle acquisition show {promise} natal foundation and {strength} current alignment. "
            "Hierarchical synthesis shows a {score}% match for vehicle purchase."
        )
