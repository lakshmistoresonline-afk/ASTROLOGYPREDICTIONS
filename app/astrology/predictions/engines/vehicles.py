...
class VehiclesPredictionEngine:
    """
    Hardened Vehicles & Mobility Engine.
    ...
    def get_prediction(chart: CanonicalChart, selected_date: datetime) -> DomainPrediction:
        ...
        if planets[l4_name].house in [1, 4, 7, 10, 5, 9, 11]:
            evidence.append(CorroborationEngine.create_evidence(
                "NATAL_PROMISE", f"MOBILITY PROMISE: Beneficial 4th Lord {l4_name} supports acquisition of vehicles.", 80.0
            ))

        # Venus (Karaka for luxury/vehicles)
        ven = planets["Venus"]
        if ven.shadbala_score > 1.1:
             evidence.append(CorroborationEngine.create_evidence(
                "MODIFIERS", "LUXURY MODIFIER: Strong Venus indicates potential for high-quality mobility.", 85.0
            ))

        # 2. DASHA ACTIVATION
        evidence.append(CorroborationEngine.create_evidence(
            "DASHA_ACTIVATION", "CURRENT ACTIVATION: Movement and asset acquisition sectors are active.", 70.0
        ))

        # 3. TIMING
        window = timing_engine.calculate_window(chart, ["Venus", "Mars", l4_name], [4, 11])
        ...
        res.practical_guidance = [
            "Verify safety features and documentation during transit peaks.",
