...
class BusinessPredictionEngine:
    """
    V3 Authoritative Business & Enterprise Engine.
    Consumes canonical evaluate_natal_promise() as the single source of structural business promise.
    """
    ...
    def get_prediction(chart: CanonicalChart, selected_date: datetime, event_type: str = None) -> DomainPrediction:
        ...
        if event_type:
            ...
            np_res = evaluate_natal_promise(chart, "BUSINESS", event_type)
        ...
        relevant_lords = [l7_name, l10_name, "Mercury", "Saturn"]
        dasha_evidence = CorroborationEngine.audit_dasha_activation(dasha, chart, relevant_lords, "business")
        if dasha_evidence:
            evidence.extend(dasha_evidence)
        else:
            evidence.append(CorroborationEngine.create_evidence(
                "DASHA_FOUNDATION",
                "STABILITY PHASE: Life-period focuses on maintenance and consolidation of enterprise operations.",
                60.0, group="SECONDARY"
            ))

        # 3. TIMING
        window = timing_engine.calculate_window(chart, ["Mercury", "Saturn", l7_name], [7, 10, 11],
        ...
        summary_template = (
            "Enterprise and business dynamics show {promise} natal foundation and {strength} current alignment. "
            "Hierarchical synthesis shows a {score}% match for commercial expansion."
        )
