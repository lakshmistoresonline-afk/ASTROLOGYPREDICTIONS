...
def main():
    ...
    domain_metrics = defaultdict(lambda: {"tp": 0, "fn": 0, "total": 0})
    for r in results:
        if r['status'] == "OCCURRED":
             domain_metrics[r['domain']]["total"] += 1
             if r['strength'] in ["ACTIVE", "PEAK"]:
                  domain_metrics[r['domain']]["tp"] += 1
             else:
                  domain_metrics[r['domain']]["fn"] += 1

    print("\n--- DOMAIN RECALL ---")
    for d, m in domain_metrics.items():
        recall = m['tp'] / m['total'] if m['total'] > 0 else 0
        print(f"  {d:20} : {recall:5.1%} ({m['tp']}/{m['total']})")
    ...
    print(json.dumps({
        ...
        "early_warning_recall": tp_watch / (tp_watch + fn_watch) if (tp_watch + fn_watch) > 0 else 0,
        "total": len(results),
