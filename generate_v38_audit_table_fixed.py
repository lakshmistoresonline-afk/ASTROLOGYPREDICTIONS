...
def run_audit():
    ...
    feature_stats = {f: {"ep": 0, "ea": 0, "sp": 0, "sa": 0} for f in FEATURE_NAMES}
    state_stats = {s: {"e": 0, "s": 0} for s in STATE_NAMES}
        ...
        try:
            ...
            for event in case["events"]:
                ...
                for k,v in DOMAIN_MAP.items():
                    if k.lower() in domain.lower(): func = v; break
                if not func: continue
                event_date = datetime.strptime(event["date"], "%Y-%m-%d")
                sim_date = event_date - timedelta(days=30)
                pred = func(chart, sim_date)
                is_event = event["status"] == "OCCURRED"
                active = {e.source for e in pred.evidence_chain}
                for f in FEATURE_NAMES:
                    p = f in active
                    if is_event:
                        if p: feature_stats[f]["ep"] += 1
                        else: feature_stats[f]["ea"] += 1
                    else:
                        if p: feature_stats[f]["sp"] += 1
                        else: feature_stats[f]["sa"] += 1
                state = pred.prediction_strength
                if state in state_stats:
                    if is_event: state_stats[state]["e"] += 1
                    else: state_stats[state]["s"] += 1
        except: pass

    print("FEATURE_DATA")
    for f in FEATURE_NAMES:
        s = feature_stats[f]
        print(f"{f}:{s['ep']}:{s['ea']}:{s['sp']}:{s['sa']}")
    print("STATE_DATA")
