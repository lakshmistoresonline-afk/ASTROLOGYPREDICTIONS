...
def run_audit():
    ...
    for case in cases:
        try:
            ...
            for event in case["events"]:
                ...
                sim_date = event_date - timedelta(days=30) # 30 day lead for prediction
                ...
                if state in state_stats:
                    if is_event: state_stats[state]["event"] += 1
                    else: state_stats[state]["stable"] += 1
        except Exception: pass

    # Table 1: Feature Discrimination
    print("### Feature Discrimination Audit (V3.8 + Transit Filter)")
    ...
    # Table 2: State Enrichment
    print("\n### State Enrichment Audit")
    print("| State | Hit Rate | Enrichment | Total Cases |")
    print("| :--- | :--- | :--- | :--- |")
    base_rate = 0.5 # 39/78 events occurred
    for s in STATE_NAMES:
        stats = state_stats[s]
        total = stats["event"] + stats["stable"]
        hr = stats["event"] / total if total > 0 else 0
        enrichment = hr / base_rate if base_rate > 0 else 1.0
        print(f"| **{s}** | {hr:.1%} | {enrichment:.2f}x | {total} |")
